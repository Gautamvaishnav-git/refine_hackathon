...
const Profile = ({imageURI, profileData, userID}: IProp) => {
    return (
        <>
            <section
                className="flex flex-col gap-3 sm:w-2/4 md:w-2/5 shadow-lg bg-secondary px-3 py-4 rounded-lg border border-primary/50 mx-auto">
                <Link href="/auth/profile/update" className="w-fit self-end" passHref>
                    <ToolTip toolTipText="edit profile">
                        <Settings
                            className="p-2 w-fit h-fit bg-transparent border-primary/30 border hover:bg-primary hover:text-primary-foreground rounded-full text-secondary-foreground duration-150"/>
                    </ToolTip>
                </Link>
                <div className=" w-fit h-fit mx-auto relative">
                    ...
                    {profileData?.id === userID && (
                        <div
                            className="w-6 h-6 rounded-full bg-green-500 absolute ring-4 ring-offset-0 ring-secondary bottom-4 right-4"></div>
                    )}
                </div>
                {profileData && (
